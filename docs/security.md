...
現行 generation 的 stopped／replacement intent，或已接受 active restart command 的 stop 子步驟，可以停止此 deployment 在同 Node 的較舊 container：要求 local mapping、nodeId/deploymentId/definition provenance 全部符合，label generation 不得高於當前 intent；不能以「舊 label」否定必要的清理，也不能跨 Node 操作。Docker labels 在 container 建立時固定，保留原 creation generation，不假裝每次 observation 都改 label；Agent journal 另記 applied generation。若 definition 被更新，只允許清理 journal 中已知的前 revision，不能 adopt 任意相似 container。

Restart 的 command-bound stop 允許 desiredState 仍為 running，但必須依 [restart execution contract](control-plane.md#restart-journal-與觀測新鮮度)，在 fresh heartbeat 確認 commandProgress 已接受、command.status=running 且未過期、activeCommandId=commandId、commandId/deploymentId/current generation 與 journal 一致、assignment 仍屬本 Node。Stop/start 各子步驟前都須重新取得 fresh intent，檢查同 generation 的 executionAuthorization.runAllowed=true、已提交且非空 reservationId、有效 resolvedImage 與相符的 container image，並先驗 ownership、durable 記錄 journal。這不是無 reservation 的 stopped intent 路徑，也不授權 remove 或重做已完成的子步驟。

Restart 中途 stopping／stopped observation 只表示 execution phase，不改 desiredState、不代表 command 完成、不釋放 reservation。只有 desiredState=stopped 且目前 generation 已確認 stopped／runtime absence 才釋放 reservation；running intent 的 runtime 暫停或 failed 仍保留 reservation 供收斂。Stop 後失聯不開始 start，重連須確認 current command/generation 仍有效及 journal 可證明下一子步驟尚未開始，再以新鮮 gates 續行；已 expired 則適用下述 recovery 邊界，結果不確定依 IndeterminateExecution 處理。Capability 自 restart 接受時撤銷，直到新的 current generation running report、fresh healthy probes 與完整 MCP discovery 通過才恢復。

Expiry 依上述 restart execution contract 的 journal phase 判定：queued 或 running/accepted 但尚未開始任何 Docker mutation 時，terminal expired 禁止 command-bound stop，交還 current intent/runtime 收斂。已開始的 Docker stop/start 子操作可完成並 journal/report；expiry 不倒轉該子操作、不證明取消，也不授權下一個 command 子步驟，terminal command 不復活。

若 command 已 expired，且 journal 證明 stop 已完成、start 尚未開始，控制交還 desired-state reconciliation，僅允許一次 start recovery。必須另取 fresh heartbeat 確認已知 expiresAt 已到期、current desiredState=running、同 current generation/assignment、activeCommandId=null，重新檢查 committed reservation、executionAuthorization.runAllowed=true、非空 reservationId、resolvedImage/container image、ownership 與 journal gates，且 inspect 確認受控 container 仍 stopped。Recovery 開始前 durable 記錄，重送不再次 start；不得再 stop、不得建立替代 restart command，舊 command 保持 expired。若 runtime 已 running 則只 report；結果不明用 IndeterminateExecution，已 expired 的遲到結果只 audit，不改寫 terminal status；新 generation 一律依最新 intent。Start recovery 不恢復舊 healthy/discovery 證據，仍需新的 current generation running report、fresh healthy probes 與完整 MCP discovery。
